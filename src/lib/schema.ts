...
export function personNode(): SchemaNode {
  ...
  return {
    ...
    // Stated from the profile rather than from `work[0]`. The most recent
    // resume entry is not necessarily a current role — reading it as one
    // published a contract that ended in January 2026 as an ongoing job.
...
export function websiteNode(): SchemaNode {
  return {
    ...
    url: HOME_URL,
    name: AUTHOR_NAME,
    alternateName: ['vaibhavjain2609.github.io', 'vaibhavjain2609'],
    description: SITE_DESCRIPTION,
