...
- **A field in `profile.json` fans out further than its readers suggest**: removing `currentCity` and `relocatingTo` also touched `Hero.tsx`, `app/contact/page.tsx`, `src/lib/telemetry.ts`, `src/data/stats/personal.tsx`, two test files, and — because `scripts/og-profile.mjs` snapshots selected fields into `public/og.meta.json` — the share card, which has to be rebuilt with `npm run og` or the committed digest no longer matches the image. Grep for the field name across `src/`, `app/`, and `scripts/` before deleting one, and rerun the build to confirm the static export no longer carries the value
- **The résumé exists in three copies and the PDF is not built by CI**: a copy change has to be made in `main.tex`, in `src/data/resume/`, and usually in `src/data/about.ts`/`Hero.tsx` where the same claim is narrated, then `public/vaibhav-jain-resume.pdf` regenerated by hand. There is no npm script for it; this machine has `tectonic` (`tectonic -X compile main.tex --outdir <dir>`) and no pdfTeX, so a rebuild changes the producer and shrinks the file by ~100KB through different font subsetting — expect that diff and confirm the result still holds one page (`pdfinfo`) and reads correctly (`pdftotext`)
- **Copy that names a client's vendor or product internals is a deliberate choice**: firewall makes, sandbox purpose, and the framing of one role as testing what another built have all been walked back after the fact. Where a bullet describes work done for an employer, prefer the capability over the brand unless the brand is the point — the skills list is where named tools belong, since it claims familiarity without attributing infrastructure to a client
...
- **Telemetry**: Measured values shown on `/stats` live in `src/lib/telemetry.ts`. Live readings use `useLiveAge`, which returns a ref rather than a value: the caller renders a **build-time reading** as the element's content and the hook assigns each later reading to `textContent`, restoring the caller's content on unmount. It deliberately does not hold the reading in state. At `AGE_PRECISION_FULL` the timer runs at the 25ms floor, so state meant 40 React renders a second; the out-of-band write survives re-renders only because what React renders never changes. That build-time value must be measured in a **server** component and passed down as a prop (`Stats/Personal.tsx` → `Stats/YearsCoding.tsx`) — calling `Date.now()` inside the client module evaluates once at export and again at hydration, and the two disagree. It must not be a digit-free placeholder: `agePlaceholder` shipped `--.-----------` under "Years writing code" to every reader without JavaScript, which is most automated readers, on the one page whose premise is that its numbers are measured. `toFixed` makes every reading the same width, so the live value cannot reflow the row. Test the server path with `renderToStaticMarkup`, since effects flush during Testing Library's `render()`, and keep the render-count test that pins the no-re-render property
