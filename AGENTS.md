...
- **Meta keywords are the only place role-targeting vocabulary belongs**: no major engine ranks on them, so aspirational terms cost nothing in the `keywords` array in `app/layout.tsx`. `SITE_DESCRIPTION`, the hero, and `src/data/resume/skills.ts` are read by people who will ask about them in an interview — a term only moves out of `keywords` into those when work backs it
