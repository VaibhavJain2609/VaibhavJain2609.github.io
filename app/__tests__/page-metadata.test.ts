...
describe('page metadata', () => {
   ...
   * every page that declares one must repeat the share image. Subpages once
   * shipped with no og:image at all for exactly this reason.
   */
  it.each([
    ['about', aboutMetadata],
    ['contact', contactMetadata],
   ...
   * rather than merged — a page that declares anything else under `alternates`
   * silently drops its canonical unless the existing object is spread first.
   */
  it.each([
    ['about', aboutMetadata, `${SITE_URL}/about/`],
    ['contact', contactMetadata, `${SITE_URL}/contact/`],
